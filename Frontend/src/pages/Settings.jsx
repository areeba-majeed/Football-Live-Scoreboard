...
function Toggle({ checked, onChange }) {
    return (
        <button
            ...
            onClick={onChange}
            className={`relative h-7 w-13.5 flex-shrink-0 cursor-pointer rounded-full p-0.5 transition-colors duration-300 ease-in-out focus:outline-none ${checked ? "bg-accent shadow-md shadow-accent/25" : "bg-slate-200 dark:bg-slate-800"
                }`}
        >
            <span
                className={`block h-6 w-6 rounded-full bg-white shadow-sm transition-transform duration-300 ease-in-out ${checked ? "translate-x-6.5" : "translate-x-0"
                    }`}
            />
        </button>
    );
            ...
            {options.map((opt) => {
                ...
                return (
                    <button
                        ...
                        className={`relative z-10 flex-1 rounded-full py-1 text-[11px] font-black capitalize transition-colors duration-300 cursor-pointer ${isActive
                                ? "text-accent"
